...
export type ApiService<T, E> = (payload?: any) => Promise<ApiResponse<T, E>> | never;
...
class HttpClient {
  constructor(){}
  public async get<T, E>(path: string, params?: RequestParam, headers?: RequestParam): Promise<ApiResponse<T, E>> | never {
    try {
  ...
  public async post<T, E>(path: string, payload: any, headers?: RequestParam): Promise<ApiResponse<T, E>> | never {
    //to do: add certs later for sslPinning
    try {
      const url = config.apiBaseUrl + path;
  ...
  private async handlePostResponse<T, E>(response: ReactNativeSSLPinning.Response): Promise<ApiResponse<T, E>> | never {
  ...
  private async handleGetResponse<T, E>(response: ReactNativeSSLPinning.Response): Promise<ApiResponse<T, E>> | never {
    const statusCode = response.status;
  ...
  public async handleFetchError(e: ReactNativeSSLPinning.Response): Promise<never> {
...
export class AuthorizedHttpClient extends HttpClient {
  ...
  protected async addHeaders(headers?: RequestParam): Promise<RequestParam> {
    ...
    return {...commonHeaders, ...headers, authorization: `Bearer ${authorTokens?.accessToken}`};
  }
}
